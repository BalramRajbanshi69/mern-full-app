...
const Profile = ({ showAddButton = true }) => {
  const params = useParams();
  const searchQuery = params.searchQuery;
  const { products, allProduct, editProduct, deleteProduct, getUserProducts } = useContext(ProductContext);
  ...
  const showRemoveNotification = () => toast.error("Removed from Cart Successfully");
    ...
    <div className="pb-8 bg-[#00171f] min-h-screen">
      <div className="container px-4 mx-auto">
        <div className="flex flex-col">
          ...
          {!displayedProducts || displayedProducts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-6 mb-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {displayedProducts.map((item) => (
                <div
                  key={item._id}
                  className="overflow-hidden bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] transform transition-all duration-300 ease-in-out hover:-translate-y-2 hover:shadow-[0_20px_35px_rgb(0,0,0,0.2)] cursor-pointer"
                >
                  <div className="relative h-48">
                    <img
                      className="object-cover w-full h-full"
                  ...
                  <div className="p-4 text-black bg-white">
                    ...
                    <div className="mt-4 space-y-1">
                      ...
                      <p className="text-sm">
                        <span className="font-semibold">InStock:</span> {item.inStock}
                      </p>
                    </div>
                    <div className="mt-4">
                      {isAuthenticated && cart.some((p) => p.productId.toString() === item._id) ? (
                        <button
                          onClick={() => {
                            showRemoveNotification();
          ...
          {!showAddButton && products?.length > visibleProducts && !showAll && (
            <div className="flex justify-center mt-8">
              <button
                onClick={handleViewAll}
                className="px-6 py-2 text-white duration-300 ease-in-out bg-blue-600 border-2 border-transparent rounded-md cursor-pointer hover:bg-transparent hover:text-white hover:border-2 hover:border-blue-600"
